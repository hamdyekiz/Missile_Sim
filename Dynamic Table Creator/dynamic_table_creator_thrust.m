...
missile.dry_mass = missile.mass - missile.fuel_mass;

% === Thrust Parameters ===
% Propellant   = I_sp value
% Solid        = 180-270
% N204/MMH     = 260-310
% N204/UDMH    = 260-310
% kerosene/L02 = 300-350
% LH2/L02      = 455
% LH2/LF       = 475
thrust.I_sp     = 300;                                                                  % specific impulse [sec]
thrust.t_b      = 5;                                                                    % burn time [sec]
thrust.T_total  = thrust.I_sp * constants.g * (missile.fuel_mass / thrust.t_b);         % total thrust [N]
...
% end   
    ...
    ud_B = (X_force + T) / missile.mass - constants.g*sin(theta);
...
function T = thrust_force(t, thrust)
    if t <= thrust.t_b
        T = thrust.T_total * (1 - t / thrust.t_b);
    else
        T = 0;
    end
end
...
function out = simulink_runner()
    ...
    % Create a SimulationInput for the chosen model
