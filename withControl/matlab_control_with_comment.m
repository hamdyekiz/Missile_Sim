% ===========================================================================
% MISSILE SIMULATION WITH CONTROL & GUIDANCE
% This is a 3-DOF missile simulation with proportional navigation guidance with three loop autopilot controller
% Authors: Burak Tasci, 
%          Furkan Balvan
%          Hamdi Ekiz
% ===========================================================================
...
% ===========================================================================
% CREATE DATA FOR SIMULINK MODEL 
% ===========================================================================
g = constants.g;
XNT = target.XNT;
V_T = target.V_T;
...
xcg = missile.xcg;
Iyy = missile.Iyy;
dry_mass = missile.dry_mass;
I_sp = thrust.I_sp;                                                                
t_b = thrust.t_b;                                                               
T_total = thrust.T_total;       

[alt_table_sim, V_a_table_sim, rho_table_sim] = read_air_data_sim();
coef_data.alt_table = alt_table_sim;
coef_data.V_a_table = V_a_table_sim;
coef_data.rho_table = rho_table_sim;

[XCG_grid_sim, ALPHA_grid_sim, MACH_grid_sim, DELTA_grid_sim, AERO_grid_sim] = read_coef_sim();

coef_data.XCG_grid   = squeeze(XCG_grid_sim(:,1,1,1));
coef_data.ALPHA_grid = squeeze(ALPHA_grid_sim(1,:,1,1));
coef_data.MACH_grid  = squeeze(MACH_grid_sim(1,1,:,1));
coef_data.DELTA_grid = squeeze(DELTA_grid_sim(1,1,1,:));

coef_data.AERO_grid = AERO_grid_sim;

% Coefficient data
coef_data.CN_data = AERO_grid_sim.CN;
coef_data.CM_data = AERO_grid_sim.CM;
coef_data.CA_data = AERO_grid_sim.CA;
coef_data.CNA_data = AERO_grid_sim.CNA;
coef_data.CMA_data = AERO_grid_sim.CMA;
coef_data.CND_data = AERO_grid_sim.CND;
coef_data.CMD_data = AERO_grid_sim.CMD;
coef_data.CMQ_data = AERO_grid_sim.CMQ;

  
simOut = simulink_runner();
...
w_i_int = 0;                              % Initial integration valuue for controller 

% Initialize state vector: [R_Tx, R_Tz, V_Tx, V_Tz, Beta, R_Mx, R_Mz, V_Mx, V_Mz, theta, q, w_i_int]
X_s = [R_Tx; R_Tz; V_Tx; V_Tz; Beta; R_Mx; R_Mz; V_Mx; V_Mz; theta; q; w_i_int];
X_o = [missile.delta];
...
while V_C >= 0  && R_TM >= 0
    ...
    [X_s, X_o] = RK4(X_s, X_o, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp);
    % ===== END SIMULATION STEP =====
...
% ===========================================================================
function [dX, ret_X_o] = missile_dynamics(X, X_o, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp)
    global forces_moment aero_prop missile;
    ...
    w_i_int =   X(12);                    % Missile controller integration value

    delta = X_o(1);     % Tail wing control surface deflection
    ...
    % Extract aero derivatives
    CA = aero_derivatives(3); 
    CMA = aero_derivatives(6);  
    CNA = aero_derivatives(7);    
    CMD = aero_derivatives(11);  
    CND = aero_derivatives(10);

    % Calcculate aero derivatives
    CN = CNA * alpha + CND * delta;
    CM = CMA * alpha + CMD * delta;
    ...
    aero_prop.aero_derivatives(2) = CM;
    aero_prop.aero_derivatives(1) = CN;
    
    missile.MACH = MACH;                  % Store Mach number
    missile.alpha = alpha;                % Store Angle of Attack
    
    % Calculate Controller Gain
    [XKR, WI, XKA, XKDC] = controller_gains(V, missile.S, missile.lref, missile.Iyy, missile.mass * constants.g, Q, CMA, CNA, CMD, CND);
    
    % Control System Calculations
    s_1 = XKDC * XNC / constants.g - wd_B_g;
    s_2 = -1 * (s_1 * XKA) + theta_d * constants.r2d;
    w_i_int_d = s_2 * WI;
    s_3 = w_i_int + theta_d * constants.r2d;
    delta = -1 * (s_3 * XKR);
    fprintf("\n======== RUN ========" + ...
              "\nXKR =      %f" + ...
              "\nWI =       %f" + ...
              ...
              "\nXKDC =     %f" + ...
              "\nDelta =    %f" + ...
              "\n", XKR, WI, XKA, XKDC, delta);
    
    % Enforce delta saturation
    delta = sign(delta) * min(19, abs(delta));

    % Assemble derivative vector for integration
    % dX = [dR_Tx/dt, dR_Tz/dt, dV_Tx/dt, dV_Tz/dt, dBeta/dt, dR_Mx/dt, dR_Mz/dt, dV_Mx/dt, dV_Mz/dt, dtheta/dt, dq/dt, d(w_i_int)/dt)]
    dX = [V_Tx; V_Tz; 0; 0; Beta_d; V_Mx; V_Mz; ud_I; wd_I; theta_d; theta_dd; w_i_int_d;];
    ret_X_o = [delta];
...
% ===========================================================================
function [X_s, X_o] = RK4(X_s, X_o, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp)
    do_RK4 = false;
    RK4_M1 = true;
    ...
    if (do_RK4 && RK4_M1)
        X_tmp = X_s;
        X_o_tmp = X_o;
        [K1, O1] = missile_dynamics(X_tmp, X_o_tmp, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp);
    
        X_tmp = X_s;
        [K2, O2] = missile_dynamics(X_tmp + 0.5 * sim.h * K1, O1, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp);
    
        X_tmp = X_s;
        [K3, O3] = missile_dynamics(X_tmp + 0.5 * sim.h * K2, O2, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp);
    
        X_tmp = X_s;
        [K4, O4] = missile_dynamics(X_tmp + sim.h * K3, O3, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp);
    
        X_s = X_s + (sim.h / 6) * (K1 + 2*K2 + 2*K3 + K4);
        X_o = (O1 + 2*O2 + 2*O3 + O4) / 6;
   elseif (do_RK4 && ~RK4_M1)
        X_tmp = X_s;
        X_o_tmp = X_o;
        [K1, ~] = missile_dynamics(X_tmp, X_o_tmp, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp);
    
        X_tmp = X_s;
        X_o_tmp = X_o;
        [K2, ~] = missile_dynamics(X_tmp + 0.5 * sim.h * K1, X_o_tmp, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp);
    
        X_tmp = X_s;
        X_o_tmp = X_o;
        [K3, ~] = missile_dynamics(X_tmp + 0.5 * sim.h * K2, X_o_tmp, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp);
    
        X_tmp = X_s;
        X_o_tmp = X_o;
        [K4, ~] = missile_dynamics(X_tmp + sim.h * K3, X_o_tmp, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp);
        
        X_s = X_s + (sim.h / 6) * (K1 + 2*K2 + 2*K3 + K4);
        X_tmp = X_s;
        X_o_tmp = X_o;
        [~, X_o] = missile_dynamics(X_tmp, X_o_tmp, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp);
    else
        X_tmp = X_s;
        X_o_tmp = X_o;
        [K1, O1] = missile_dynamics(X_tmp, X_o_tmp, sim, constants, thrust, target, AERO_grid, V_air_interp, rho_interp);
...
% ===========================================================================
% READ AERODYNAMIC COEFFICIENT DATA for MATLAB
...
function [XCG_grid, ALPHA_grid, MACH_grid, DELTA_grid, AERO_grid] = read_coef()
    ...
    % === Step Compute CND and CMD using central difference ===
...
% ===========================================================================
% READ ATMOSPHERIC DATA FOR MATLAB
...
% ===========================================================================
% READ AERODYNAMIC COEFFICIENT DATA for SIMULINK
% Loads aerodynamic coefficient lookup tables from CSV files
% ===========================================================================
function [XCG_grid, ALPHA_grid, MACH_grid, DELTA_grid, AERO_grid] = read_coef_sim()
    folderPath = fullfile(pwd, 'coef_output_def');
    xcgDirs = dir(folderPath);
    xcgDirs = xcgDirs([xcgDirs.isdir] & ~startsWith({xcgDirs.name}, '.')); 
...
% ===========================================================================
%  CONTROLLER GAIN CALCULATION FUNCTION
% ===========================================================================
function [XKR, WI, XKA, XKDC] = controller_gains(V_M, SREF, DIAM, XIYY, WGT, Q, CMA, CNA, CMD, CND)
    % Useful Constants
    r2d = 180/pi;                             % Conversion Rad to Deg
    d2r = pi/180;
    g = 9.81;                                 % Gravitational constant (m/s^2)

    % Align coefficient units
    CMD = CMD / d2r;
    CND = CND / d2r;
    ...
    ZA = -g*Q*SREF*CNA/((WGT)*V_M);             % Z_∝ (1/s)
    ZD = -g*Q*SREF*CND/((WGT)*V_M);             % Z_δ (1/s)
    ...
    XK3 = r2d*g*XK1/V_M;                      % Gain K3: Aerodynamic body rate gain (1/s)
    
    W = (TAU*WCR*(1+2.*ZAF*WAF/WCR)-1)/(2*ZETA*TAU); % Autopilot design parameter 
...
% ===========================================================================
% READ ATMOSPHERIC DATA FOR SIMULINK
% Parse air velocity and density tables for atmospheric modeling
% ===========================================================================
...
function [saved_data] = save_data(sim, X_s, missile, forces_moment, aero_prop, R_TMx, R_TMz, V_TMx, V_TMz, Lamda, Lamda_d, V_C, XNC,alpha)
    ...
    persistent alpha_all rho_all time_all xcg_all mass_all MACH_all Iyy_all delta_all;
...
% ===========================================================================
% ==== FUNCTION FOR RUN SIMULINK MODEL====
% ===========================================================================
function out = simulink_runner()
    modelName = "proportionalNav_withControlSystem";
